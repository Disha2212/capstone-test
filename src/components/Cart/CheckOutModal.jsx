...
const CheckOutModal = ({ toggleModal, pendingOrders, totalAmount }) => {
  ...
  const handleWithStripe = async () => {
    try {
      const stripe = await loadStripe(
        "pk_test_51NJwzHCdB1SDHMEBIdAAZWEDMrFP5CSxKteFAWoakd6iiLrPODZyOMH29brjImYKSN7Tbk0lSgR8Ep0I6fduvxbP00kbdWWLNU"
      );
      ...
      const response = await fetch("http://localhost:5400/api/checkout_sessions", {
        method: "POST",
        headers: headers,
      ...
      {complete === true ? (
        ...
        <>
          <div className={modalcss.left_section}>
            ...
            <button className={modalcss.confirm_button} onClick={handleSubmit}>
              Confirm Order
            </button>
          </div>
          ...
          <div className={modalcss.right_section}>
            <div className={modalcss.charges_section}>
              ...
              <button
                className={modalcss.charges_section}
                onClick={handleWithStripe}
              >
                Pay With Stripe
              </button>
            </div>
